...
type Viewport() =
    ...
    let mutable surfaceMutex = Unchecked.defaultof<SurfaceLock>

    member this.SurfaceMutex
        with get () = surfaceMutex
        and set value =
            this.SetAndRaise(Viewport.SurfaceMutexProperty, &surfaceMutex, value)
            |> ignore

            viewport <- Rect(Size(float surfaceMutex.Surface.Width, float surfaceMutex.Surface.Height))

    static member SurfaceMutexProperty: DirectProperty<Viewport, SurfaceLock> =
        AvaloniaProperty.RegisterDirect<Viewport, SurfaceLock>(
...
and DrawRender(bounds: Rect, surfaceMutex: SurfaceLock) =
        ...
        member this.Render(context) =
            ...
            let dest =
                SKRect.Create(
                    bounds.X |> float32,
                    bounds.Y |> float32,
                    bounds.Width |> float32,
                    bounds.Height |> float32
                )
...
type Viewport with
    static member create attrs = ViewBuilder.Create<Viewport>(attrs)

    static member surface<'t when 't :> Viewport>(value: SurfaceLock) =
        AttrBuilder<'t>
            .CreateProperty<SurfaceLock>(Viewport.SurfaceMutexProperty, value, ValueNone)
